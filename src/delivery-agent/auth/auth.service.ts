...
import { DeliveryAgentActivationTokenPayload, JwtPayload } from './jwt-payload.interface';
...
export class DeliveryAgentAuthService {
    ...
    async registerDeliveryAgent(email: string, password: string, name: string, phoneNumber: string) {
        ...
        const payload: DeliveryAgentActivationTokenPayload = { email, name, password: hashedPassword, phoneNumber, code: activationCode };
    ...
    async verifyDeliveryAgent(token: string, activationCode: string) {
        try {
            const decoded: DeliveryAgentActivationTokenPayload = this.jwtService.verify(token);
            ...
            const payload: JwtPayload = { email: user.email, sub: user.id, role: "DELIVERY_AGENT" };
    ...
    async verifyGoogleIdTokenDeliveryAgent(idToken: string) {
        ...
        const ticket = await new OAuth2Client(process.env.GOOGLE_CLIENT_MOBILE_DELIVERY_ID).verifyIdToken({
            idToken,
