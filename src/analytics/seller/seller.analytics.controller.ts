...
export class SellerAnalyticsController {
  ...
  async getSellerAnalytics(@GetUser('userId') userId: string) {
    ...
    const [totalRevenue, subscriptions, sales, recentSales, monthlyData] =
      await Promise.all([
        this.sellerAnalyticsService.getTotalRevenue(
          userId,
          currentMonth,
          currentYear,
        ),
        this.sellerAnalyticsService.getTotalSubscriptions(
          userId,
          currentMonth,
          currentYear,
        ),
        this.sellerAnalyticsService.getTotalSales(
          userId,
          currentMonth,
          currentYear,
        ),
        this.sellerAnalyticsService.getRecentSales(
          userId,
          currentMonth,
          currentYear,
        ),
