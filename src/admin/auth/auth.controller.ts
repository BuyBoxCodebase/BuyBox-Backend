...
import { BadRequestException, Controller, Get, Post, Req, Res, UploadedFiles, UseGuards, UseInterceptors } from '@nestjs/common';
  ...
  async googleAuthCallback(@Req() req, @Res() res: Response) {
    ...
    req.login(req.user, (err) => {
      if (err) {
        ...
        return res.redirect('https://localhost:5173/admin?auth=error');
      }
      ...
      req.session.save((err) => {
        if (err) {
          ...
          return res.redirect('https://localhost:5173/admin?auth=failed');
        }
        ...
        return res.redirect('https://localhost:5173/admin?auth=success');
      });
    });
  }
